...
GREEN='\033[00;32m'
YELLOW='\e[0;33m'
HOST_PROD=my_project.example.com
...
function dknpminstall {
    CD=$(pwd)
    cd $PROJ_BASE
    docker run -it --rm -v $(pwd):/app -w /app/frontend -e NODE_ENV=development my_project npm install
    exitcode=$?
    cd $CD
    return $exitcode
}
...
function dkrun_prod {
    ...
    docker run --name my_project -d --env-file /home/ubuntu/my_project.env \
        -p 3000:3000 -p 8000:8000 \
        -v /home/ubuntu/dkdata/my_project:/dkdata \
        my_project start_web.sh
}

function deploy_prod {
  rsync -av --exclude frontend/node_modules --exclude frontend/.nuxt * ubuntu@$HOST_PROD:./my_project/
...
echo_green "Welcome to my_project's dev env"
