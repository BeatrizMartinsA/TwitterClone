...
```bash
dk bash  # starts bash inside "my_project" container
...
* `dkup` uses docker-compose to start 3 containers: postgres, nginx, and my_project.
...
* Once `dkup` is running, `dk <command>` will run `<command>` inside the `my_project` container. So `dk bash` will get you "logged in" as root inside that container. Once inside, you need to run Django's `manage.py` commands to initialize the database properly.
* The my_project container runs 3 services:
 * django on port 8000
...
```bash
mkvirtualenv my_project -p python3  # creates a python3 virtualenv
...
Install docker and nginx. Create an empty postgres database my_project owned by a user my_project.

On your remote machine, create a file ~/my_project.env:

```
DJANGO_DB_PASSWORD=<my_project's password>
DJANGO_DB_HOST=<database_ip>
DJANGO_DB_NAME=my_project
DJANGO_DB_USER=my_project
DJANGO_DEBUG=0
```
...
```
server {
    server_name  my_project.example.com;
    ...
    location /static {
        alias /home/ubuntu/dkdata/my_project/static;
...
(Replace "my_project.example.com" with your production domain)
