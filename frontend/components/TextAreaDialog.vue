  ...
  <div>
    <v-dialog v-model="visible" width="640">
      <v-card>
        ...
        <v-btn class="blue--text darken-1" rounded @click="close()">Cancelar</v-btn>
        <v-btn class="blue--text darken-1" rounded @click="ok()" :loading="loading" :disabled="loading">{{action}}</v-btn>
      </v-card>
    </v-dialog>
  </div>
</template>
